Create Codex pet row `running-right` for `jinling-duck`: exactly 8 full-body frames in one horizontal strip on flat pure blue #0000FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 金陵鸭鸭: chibi explorer duck, white duck, safari hat, scarf, map, bag, cute, simple, no background; inspired by Jinling/Nanjing travel motifs, tiny city-wall charm or plum blossom accent allowed, no readable text. Preserve silhouette, face, palette, material, proportions, markings, and props.
